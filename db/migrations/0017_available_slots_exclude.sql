drop function if exists available_slots(text, text, text, date, text);
--> statement-breakpoint
-- Which start times can this service be booked at, on this date, at this
-- branch?
--
-- Replaces the 0015 definition, adding p_exclude_booking_id for rescheduling.
-- `create or replace` cannot add a parameter to an existing function -- the
-- signature is part of its identity, so the old five-argument version would
-- survive alongside this one and callers would resolve to whichever matched.
...
create or replace function available_slots(
  ...
  -- Rescheduling asks "where else could THIS booking go?", so its own row must
  -- not count as busy: moving a 60-minute appointment from 10:00 to 10:30
  -- otherwise collides with itself and every nearby slot reads as taken.
  -- Null for every other caller, which is the normal case.
